...
const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    const totalDiscount = cartItem.reduce((acc, item) => acc + item.total_price, 0);
...
console.log(cartItem);
    ...
    const increaseQuantity = (id) => {
      ...
      const discountItem = cartItem.filter(item=> id === item._id)
      const discountItemPrice = discountItem[0].total_price;
      
      setDiscountPrice(discountPrice+discountItemPrice);
